...
class EventProcessingThread(KillableThread, ServiceConnection):
	...
	def updateActionProcessor(self):
		...
		try:
			...
			System.execute('"%s" -on "%s" -ot file -actn trustee -trst n1:S-1-5-32-545;s1:y;ta:remtrst;w:dacl"' % (setaclcommand,  actionProcessorLocalDir))
			logger.notice(u"therefore set new rights")
			System.execute('"%s" -on "%s" -ot file -actn ace -ace "n:S-1-5-32-544;p:full;s:y" -ace "n:S-1-5-32-545;p:read_ex;s:y" -actn clear -clr "dacl,sacl" -actn rstchldrn -rst "dacl,sacl"' % (setaclcommand,  actionProcessorLocalDir))
