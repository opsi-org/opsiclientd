...
def test_start_stop_notification_server() -> None:
	...
	notification_server1 = NotificationServer(address=address, start_port=start_port, subjects=subjects)  # ty: ignore[invalid-argument-type]
	notification_server1.start()
	notification_server1.wait_ready(5)
	assert notification_server1.port == start_port

	notification_server2 = NotificationServer(address=address, start_port=start_port, subjects=subjects)  # ty: ignore[invalid-argument-type]
...
def test_notification_server_multi_client() -> None:
	...
	choice_subject.setChoices(["abort", "start"])
	subjects = [choice_subject]
	notification_server = NotificationServer(address=address, start_port=start_port, subjects=subjects)  # ty: ignore[invalid-argument-type]
