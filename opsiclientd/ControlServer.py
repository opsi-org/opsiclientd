...
class OpsiclientdRpcInterface(OpsiclientdRpcPipeInterface):
	...
	def runAsTest(self, command, maxWait=120000):
		try:
			import os
			import msvcrt
			import win32security
			...
			userSid = win32security.LookupAccountName(None, user_info["name"])[0]
			# Login as domain user and create new session
			userToken = win32security.LogonUser(user_info["name"], None, user_info["password"],
